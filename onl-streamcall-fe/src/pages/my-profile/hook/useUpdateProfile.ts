...
import { updateProfile, updateProfileWallet } from "@/api/profile";
...
export const useUpdateProfiles = () => {
  ...
  const updateWallet = async () => {
    setIsLoading(true);
    try {
      const res = await updateProfileWallet({ address: walletAddress });

      if (res.status === 200) {
        dispatch(
          notify({
            text: "Update successfully!",
            title: "Update account’s wallet address",
          })
        );
        setIsViewMode(true);
      }
  ...
  const walletActionManager = () => {
    if (walletActionButtonState === "NOT-CONNECT") {
      connectWalletHandler();
    } else if (walletActionButtonState === "UPDATE-WALLET") {
      updateWallet();
    }
  };
