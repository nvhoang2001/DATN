...
import { IFormLogin, IFormRegister, ILoginMetamask, IProfile } from "@/interfaces/type";
import { createAsyncThunk } from "@reduxjs/toolkit";
import { _login, _loginWallet, _register } from "@/api/auth";
...
const loginWithMetamask = createAsyncThunk(
  "/auth/login-wallet",
  async (params: ILoginMetamask, { rejectWithValue }) => {
    try {
      const data = await _loginWallet(params);
      return { data: data };
    } catch (error) {
      return rejectWithValue(error);
    }
...
export const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(loginWithMetamask.fulfilled, (state, { payload }) => {
        state.isLogged = true;
        Cookies.set(COOKIES_STORAGE_KEY.ACCESS_TOKEN, payload.data.access_token);
      })
